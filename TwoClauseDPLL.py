    ...
    while HasUnit:
        ...
        for clause in formula:
            if len(clause) == 1:
                ...
                TwoClauses[abs(clause[0])] += 1
                TwoClauses[abs(clause[1])] += 1
        if np.max(IsUnit * TwoClauses) > 0:
            prop = np.argmax(IsUnit * TwoClauses)
        else:
...
def TCDPLL(N, L, cnf, lit, unassigned, splitting, starttime):
    TimeLimit = 60
    #HasNull = False
    Satisfied = False
    #lit = [None] * (N + 1)
